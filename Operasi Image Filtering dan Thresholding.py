...
img = cv2.imread("riram.jpg") # Membaca gambar dengan nama file "riram.jpg" menggunakan OpenCV, yang menghasilkan citra dengan format BGR.

riram = cv2.cvtColor(img, cv2.COLOR_BGR2RGB) # Mengubah format citra dari BGR menjadi RGB menggunakan fungsi cv2.cvtColor.
...
riram_filter = cv2.filter2D(img,-1,kernel)
...
plt.subplot(222), plt.imshow(riram)
plt.title('Original')
plt.xticks([]), plt.yticks([])

plt.subplot(223), plt.imshow(riram_filter)
plt.title('Averaging')
plt.xticks([]), plt.yticks([])

plt.subplot(224)
hist_data = riram_filter.flatten()
...
riram_blur = cv2.blur(img,(5,5))
plt.imshow(riram_blur)
...
img = cv2.imread("riram.jpg", 0)  # Membaca citra dengan nama file "riram.jpg" dalam format grayscale menggunakan OpenCV, dengan memberikan argumen 0 kepada fungsi `cv2.imread`.
